...
def load_network(net, label, epoch, opt):
    if 'D' in label and not opt.same_netD_model and opt.unpairTrain:
        save_filename = '%s_net_%s2.pth' % (epoch, label)
    else:
        save_filename = '%s_net_%s.pth' % (epoch, label)
...
def load_blend_network(net, label, epoch, opt):
    save_filename = '%s_net_%s.pth' % (epoch, label)
    save_dir = os.path.join(opt.checkpoints_dir, opt.name)
    save_path = os.path.join(save_dir, save_filename)
...
def load_inpainting_network(net,  opt):
    save_filename = opt.ig_model_name
    save_dir = os.path.join(opt.checkpoints_dir, opt.name)
    save_path = os.path.join(save_dir, save_filename)
...
def load_sinpainting_network(net,  opt):
    save_filename = opt.sig_model_name
    save_dir = os.path.join(opt.checkpoints_dir, opt.name)
    save_path = os.path.join(save_dir, save_filename)
    print(save_path)
    if torch.cuda.is_available():
...
def blend_image(fake, tag, mask):
    fake_mask = np.ones_like(fake.shape, fake.dtype)
    fake_mask = fake_mask * mask[...,np.newaxis] * 255
    fake_mask = fake_mask.astype(fake.dtype)
    ...
    out = cv2.seamlessClone(fake, tag, fake_mask, center, cv2.MIXED_CLONE)
    return out
