...
class Visualizer():
    ...
    def print_current_errors(self, epoch, i, errors, t, curr_size=None):
        ...
        if curr_size is not None:
            message = '(step: %d, epoch: %d, iters: %d, time: %.3f, size: %d) ' % (self.opt.curr_step, epoch, i, t, curr_size)
        else:
            message = '(step: %d, epoch: %d, iters: %d, time: %.3f) ' % (self.opt.curr_step, epoch, i, t)
