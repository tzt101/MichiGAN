...
from models.networks.normalization import SPADE, SPADEImage
import torch.nn.utils.weight_norm as weight_norm_0
from models.networks.normalization import weight_norm as weight_norm_1
...
class SPADEResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt):
        ...
        if self.learned_shortcut:
            self.norm_s = SPADE(spade_config_str, fin, norm_nc, opt.weight_norm_G)
...
class SPADEImageBlock(nn.Module):
    def __init__(self, fin, fout, opt, downsample_n):
        ...
        if self.learned_shortcut:
            self.norm_s = SPADEImage(spade_config_str, fin, 3, downsample_n)

    # note the resnet block with SPADE also takes in |seg|,
    # the semantic segmentation map as input
    def forward(self, x, image):
        x_s = self.shortcut(x, image)

        dx = self.conv_0(self.actvn(self.norm_0(x, image)))
        dx = self.conv_1(self.actvn(self.norm_1(dx, image)))

        out = x_s + dx

        return out

    def shortcut(self, x, image):
        if self.learned_shortcut:
            x_s = self.conv_s(self.norm_s(x, image))
        else:
            x_s = x
        return x_s
